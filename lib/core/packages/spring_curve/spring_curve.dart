...
class ElegantSpring extends Curve {
  ElegantSpring({this.bounce = 0.0}) {
    assert(
      ...
      '"bounce" value must be between 0.0 and 1.0 inclusive',
    );

    _dampingRatio =
        _maxDampingRatio -
        (sqrt(bounce) * (_maxDampingRatio - _minDampingRatio));
  ...
  late final double _sinusMultiplication =
      _dampingCoefficient / _angularFrequency;
  ...
  /// Mimics windows motion animations. (use with 1s duration)
  static final ElegantSpring windows = ElegantSpring(bounce: 0.7);
